...
import { Settings, Heart, Star, MessageSquare, Calendar, CheckCircle2, ShieldCheck, Gamepad2, ArrowRight, Users } from "lucide-react";
...
export default async function ProfilePage() {
  ...
  const [userRecord, reviews, favorites, forumThreads] = await Promise.all([
      ...
      <section className="space-y-4 pt-6">
        ...
        {forumThreads.length === 0 ? (
          <div className="rounded-2xl border border-dashed p-10 text-center">
            ...
            <p className="font-medium text-sm mb-1">No forum activity yet</p>
            <p className="text-xs text-muted-foreground mb-4">Start or join discussions on game forums.</p>
            <Button size="sm" render={<Link href="/forum" />} className="gap-2">
              Browse Forums
            </Button>
          </div>
        ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Badge variant="secondary" className="text-xs shrink-0">
                      {thread.authorId === userId ? "Started" : "Replied"}
                    </Badge>
                  </div>
